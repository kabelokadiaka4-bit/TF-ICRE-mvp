...
module "artifact_registry" {
  ...
  description   = "Docker images for TF-ICRE microservices in dev environment"
  environment   = "dev"
}
...
module "vpc_connector" {
  ...
  network_name  = module.network.network_name
  ip_cidr_range = "10.8.0.0/28" # Dedicated IP range for connector
}
...
resource "google_composer_environment" "orchestrator" {
  name    = "tf-icre-composer-dev" # Name includes dev for clarity
  project = var.gcp_project_id
  region  = var.gcp_region
